...
Yii::import('yiibooster.widgets.YbEditableField');
Yii::import('zii.widgets.grid.CDataColumn');

class YbEditableColumn extends CDataColumn
{
    //editable params
    ...
    public function init()
    {
        if (!$this->grid->dataProvider instanceOf CActiveDataProvider) {
            throw new CException(Yii::t('zii', 'YbEditableColumn can be applied only to grid based on CActiveDataProvider'));
        }
        if (!$this->name) {
            throw new CException(Yii::t('zii', 'You should provide name for YbEditableColumn'));
        }
    ...
    protected function renderDataCellContent($row, $data)
    {
        ...
        $editable = $this->grid->controller->createWidget('YbEditableField', $options);
